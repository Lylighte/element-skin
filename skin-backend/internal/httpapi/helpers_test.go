...
func TestPublicBoolAndValidation(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := validPublicValue(tc.input); got != tc.valid {
				t.Fatalf("validPublicValue(%#v)=%v, want %v", tc.input, got, tc.valid)
			}
			if got := publicBool(tc.input); got != tc.value {
				t.Fatalf("publicBool(%#v)=%v, want %v", tc.input, got, tc.value)
			}
		})
	}
}
...
func TestCursorCreatedHashParsesExactKeys(t *testing.T) {
	...
	created, hash, err := cursorCreatedHash(cursor, "last_skin_hash")
	if err != nil {
		t.Fatal(err)
	}
	...
	created, hash, err = cursorCreatedHash("", "last_skin_hash")
...
func TestParseImportProfilesValidatesShapeAndTrimsValues(t *testing.T) {
	...
	for _, raw := range []any{nil, "not-list", []any{}, []any{"not-map"}} {
		if _, err := parseImportProfiles(raw); err == nil {
			t.Fatalf("parseImportProfiles(%#v) should reject invalid shape", raw)
		}
	}
}
