...
func TestAccountServiceBanUserPersistsInvalidatesCacheAndSendsExactNotice(t *testing.T) {
	...
	svc := adminsvc.AccountService{DB: db, Redis: cache}
	...
	gotUntil, err := svc.BanUser(ctx, actor, target.ID, adminsvc.BanUserInput{BannedUntil: bannedUntil, Reason: "  " + reason + "  "})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountServiceBanUserRejectsInvalidInputsWithoutMutation(t *testing.T) {
	...
	svc := adminsvc.AccountService{DB: db, Redis: redisstore.NewMemoryStore()}
	actor := actorWithPermissions(adminUser.ID, "account.ban.any")
	future := database.NowMS() + int64(time.Hour/time.Millisecond)

	if _, err := svc.BanUser(ctx, permission.Actor{}, target.ID, adminsvc.BanUserInput{BannedUntil: future, Reason: "reason"}); !httpErrorIs(err, http.StatusForbidden, "permission denied") {
		t.Fatalf("ban without permission error mismatch: %#v", err)
	}
	if _, err := svc.BanUser(ctx, actor, target.ID, adminsvc.BanUserInput{BannedUntil: 1, Reason: "reason"}); !httpErrorIs(err, http.StatusBadRequest, "banned_until is required") {
		t.Fatalf("expired ban timestamp error mismatch: %#v", err)
	}
	if _, err := svc.BanUser(ctx, actor, target.ID, adminsvc.BanUserInput{BannedUntil: future, Reason: " \t\n "}); !httpErrorIs(err, http.StatusBadRequest, "reason is required") {
		t.Fatalf("missing ban reason error mismatch: %#v", err)
	}
	if _, err := svc.BanUser(ctx, actor, target.ID, adminsvc.BanUserInput{BannedUntil: future, Reason: strings.Repeat("中", 501)}); !httpErrorIs(err, http.StatusBadRequest, "reason too long") {
...
func TestAccountServiceProtectsSuperAdminAndUnbansExactly(t *testing.T) {
	...
	future := database.NowMS() + int64(time.Hour/time.Millisecond)
	if _, err := svc.BanUser(ctx, plainActor, protectedAdmin.ID, adminsvc.BanUserInput{BannedUntil: future, Reason: "protected"}); !httpErrorIs(err, http.StatusForbidden, "cannot modify super admin") {
...
func TestAccountServicePropagatesClosedDatabaseErrors(t *testing.T) {
	...
	if _, err := svc.BanUser(ctx, actor, adminUser.ID, adminsvc.BanUserInput{BannedUntil: database.NowMS() + int64(time.Hour/time.Millisecond), Reason: "closed"}); err == nil || !strings.Contains(err.Error(), "closed pool") {
...
func TestAccountServiceProtectedManagerCanBanProtectedUser(t *testing.T) {
	...
	if _, err := svc.BanUser(ctx, actor, protected.ID, adminsvc.BanUserInput{BannedUntil: bannedUntil, Reason: "protected managed"}); err != nil {
		t.Fatal(err)
	}
