...
func TestRedisStoreOAuthAccessTokenTargetedInvalidationExactly(t *testing.T) {
	...
	assertRedisOAuthAccessPresence(t, store, map[string]bool{
...
func assertRedisOAuthAccessPresence(t *testing.T, store Store, want map[string]bool) {
	t.Helper()
	for hash, present := range want {
		...
		if !present && !errors.Is(err, ErrCacheMiss) {
