...
func TestSettingsVerificationAndFallbackModules(t *testing.T) {
	...
	val, err := db.Settings.Get(ctx, "enable_skin_library", "")
	if err != nil || val != "true" {
		t.Fatalf("enable_skin_library=%q err=%v", val, err)
	}
	missing, _ := db.Settings.Get(ctx, "non_existent_key", "default_val")
	if missing != "default_val" {
		t.Fatalf("default setting mismatch: %q", missing)
	}
	if err := db.Settings.Set(ctx, "test_key", "test_value"); err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Verifications.CreateCode(ctx, "test@verify.com", "ABCDEFGH", "register", 1); err != nil {
		t.Fatal(err)
	}
	code, expiresAt, ok, err := db.Verifications.GetCode(ctx, "test@verify.com", "register")
	...
	if err := db.Verifications.DeleteCode(ctx, "test@verify.com", "register"); err != nil {
		t.Fatal(err)
	}
	_, _, ok, _ = db.Verifications.GetCode(ctx, "test@verify.com", "register")
	if ok {
		t.Fatal("verification code should be deleted")
	}

	eps, err := db.Fallbacks.ListEndpoints(ctx)
	if err != nil {
		t.Fatal(err)
	}
	if len(eps) != 0 {
	...
	primary, err := db.Fallbacks.PrimaryEndpoint(ctx)
	if err != nil || primary["note"] != "CustomEP" {
		t.Fatalf("unexpected primary: %#v err=%v", primary, err)
	}
	if err := db.Fallbacks.AddWhitelistUser(ctx, "WhitelistedPlayer", endpointID); err != nil {
		t.Fatal(err)
	}
	in, err := db.Fallbacks.IsUserInWhitelist(ctx, "WhitelistedPlayer", endpointID)
	if err != nil || !in {
		t.Fatalf("expected whitelist hit: %v", err)
	}
	miss, _ := db.Fallbacks.IsUserInWhitelist(ctx, "NonExistent", endpointID)
	if miss {
		t.Fatal("unexpected whitelist hit")
	}
	users, err := db.Fallbacks.ListWhitelistUsers(ctx, endpointID)
	if err != nil || len(users) != 1 || users[0]["username"] != "WhitelistedPlayer" {
		t.Fatalf("unexpected whitelist users: %#v err=%v", users, err)
	}
	if err := db.Fallbacks.RemoveWhitelistUser(ctx, "WhitelistedPlayer", endpointID); err != nil {
		t.Fatal(err)
	}
	in, _ = db.Fallbacks.IsUserInWhitelist(ctx, "WhitelistedPlayer", endpointID)
	if in {
