...
```javascript
{
	...
	"signaturePublickey":"本站用于验证数字签名的公钥",
	"signaturePublickeys":[
		"本站及 fallback 用于验证数字签名的公钥"
		// ,...
	]
}
```
...
Element Skin 保留单数 `signaturePublickey` 以兼容既有客户端，该字段始终只返回本站公钥。复数 `signaturePublickeys` 的第一项同样是本站公钥，后续按 fallback 优先级返回缓存的远端公钥，并按规范化 DER 内容去重。
...
响应中的 `publicKey` 是不带 PEM 头尾和换行的 Base64 编码 SubjectPublicKeyInfo DER：

```json
{
  "playerCertificateKeys": [
    { "publicKey": "MIICIjANBgkqh..." }
  ],
  "profilePropertyKeys": [
    { "publicKey": "MIICIjANBgkqh..." }
  ]
...
两个数组均以本站公钥为第一项；fallback 公钥按 fallback 端点优先级追加，并在各自数组内去重。该端点与根元数据均属于公开读取能力：无 Cookie 和 OAuth token 时使用 Guest Actor，有有效凭证时使用真实 Actor，无效凭证返回 `401`。

### fallback 公钥发现与缓存

fallback 公钥刷新与 Services 健康检查是两条独立流程。原有 `services_url/minecraft/profile/lookup/name/jeb_` 健康检查地址和判定逻辑保持不变。

每个 fallback 端点按以下顺序刷新公钥：

1. 计算 `session_url`、`account_url`、`services_url` 三个 URL 的最长公共路径根；三者协议或主机不同、URL 非法时视为无法使用发现入口。
2. 若存在发现入口，请求该根地址并解析 `signaturePublickey` 与 `signaturePublickeys`。响应成功、格式合法且至少包含一个 RSA 公钥时立即采用结果，不再请求 Services 公钥端点。
3. 发现入口请求失败、解析失败或没有公钥时，请求 `services_url/publickeys/`，解析 `playerCertificateKeys` 与 `profilePropertyKeys`。
4. 两种方式均失败时保留该来源已有缓存，不写入空结果，也不改变 fallback 健康状态。

成功结果按来源写入 Redis，缓存有效期为 48 小时。同一来源在一轮刷新中只请求一次；fallback 配置变更时清除旧来源缓存。对外响应只聚合当前启用 fallback 端点对应的缓存，因此已移除端点的旧缓存不会继续暴露。
