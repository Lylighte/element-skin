...
- 首页背景支持普通图片和 Minecraft 全景图，媒体可以配置启用状态、轮播时长、浅色/深色遮罩透明度。
- 全景图以六面 PNG ZIP 上传，严格校验 `panorama_0.png` 到 `panorama_5.png` 的文件集合、压缩包大小和单面大小。
- 全景首页使用 WebGL 场景加载立方体贴图，支持起始 yaw/pitch、水平/垂直旋转速度、面序转换、渐变切换和预览裁切。
- 管理员可以在首页媒体管理中上传、预览、排序、修改和删除普通图片及全景图。
- 全景图素材通过独立的媒体存储路径提供，公开首页读取缓存后的媒体配置，不要求前端重新编译。
- 首页渲染限制更新频率并减少无效渲染，降低高分辨率背景的资源消耗。

### 3. 节日彩蛋系统

- 首页和站点布局新增可配置的节日彩蛋系统，彩蛋模块按需 lazy import，不会在每次加载时加载全部节日效果。
- 管理员可以在“彩蛋列表”中配置服务端允许启用的彩蛋，配置使用结构化的 `enabled_easter_eggs` 数据表保存，并通过公开站点设置返回当前启用标识。
- 客户端会同时检查服务端允许列表、本地日期规则和用户个人“关闭彩蛋”设置，三个条件都满足时才启动效果。
- 支持的彩蛋包括：春节、愚人节、清明、儿童节、端午节、Minecraft 周年、中秋节、万圣节和圣诞节。
- 不同彩蛋提供独立的节日主题效果，包括首页配色、点击火花、背景烟花、物理交互、细雨、彩色气泡、粽子重力与碰撞、像素成就框、月光与桂花粒子、南瓜灯 hover 效果以及飘雪。
- 彩蛋在路由切换、主题切换和配置刷新时能够清理旧 DOM、样式、Canvas 和动画，避免效果叠加或离开首页后残留。
- 用户可以在个人资料中关闭彩蛋；关闭设置使用统一浏览器存储抽象保存，不影响其他用户或服务端配置。
